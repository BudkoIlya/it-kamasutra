...
const ProfileDataForm: React.FC<InjectedFormProps<ProfileType, PropsType> & PropsType> = ({
  ...
  <form onSubmit={handleSubmit}>
    ...
    <div>
      ...
      {CreateField<LoginFormValuesTypeKeys>('', 'lookingForAJob', [], Input, { type: 'checkbox' })}
    </div>
    <div>
      <b>My Professional skills</b>
      {CreateField<LoginFormValuesTypeKeys>('My Professional skills', 'lookingForAJobDescription', [], Textarea)}
    </div>
    <div>
      <b>About me</b>: {CreateField<LoginFormValuesTypeKeys>('About me', 'aboutMe', [], Textarea)}
    </div>
    <div>
      <b>Contacts</b>:
      {Object.entries(profile.contacts).map((contact, idx) => (
        <div key={String(idx)}>
          ...
          {contact[0]}: {CreateField('Enter contact', `contacts.${contact[0]}`, [], Input)}
        </div>
      ))}
    </div>
  </form>
